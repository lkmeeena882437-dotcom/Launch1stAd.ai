...
const steps = ["Business", "Promote", "Audience", "Generate"];
...
export function CampaignForm({ form, update, onSubmit }: {
  ...
  return (
    <section className="rounded-2xl bg-card p-5 md:p-8">
      ...
      <p className="mt-4 leading-7 text-muted">Business, destination, audience, budget aur payment model select karo.</p>
      ...
      <form onSubmit={submit} className="mt-8 space-y-5">
        ...
        {step === 4 && (
          <div className="rounded-2xl bg-canvas p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-muted">Report will include platforms, destination, audience category, budget model, ad copy and 7-day plan.</p>
            <div className="mt-4 grid gap-3 text-sm md:grid-cols-2">
              ...
              <p><span className="font-semibold">Payment:</span> {form.paymentModel}</p>
            </div>
          </div>
        )}
