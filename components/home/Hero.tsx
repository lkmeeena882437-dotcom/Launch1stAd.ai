  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-5 py-10 lg:grid-cols-[1fr_0.85fr] lg:items-center md:py-16">
      <div className="rounded-[2rem] bg-white/80 p-6 backdrop-blur md:p-10">
        <p className="inline-flex rounded-full border border-hairline bg-card px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-coral">AI advertising workspace</p>
        <h1 className="mt-5 max-w-3xl text-4xl font-black leading-[1.05] tracking-tight text-ink md:text-6xl">Plan, create and manage better ad campaigns.</h1>
        <p className="mt-5 max-w-2xl text-base leading-7 text-muted md:text-lg">Launch1stAd.ai helps businesses build campaign briefs, generate ad copy, prepare targeting and review spend from one workspace.</p>
        ...
        <div className="mt-7 grid gap-3 text-sm text-muted sm:grid-cols-3">
          <div className="rounded-2xl border border-hairline bg-white p-4">✓ Campaign planning</div>
          <div className="rounded-2xl border border-hairline bg-white p-4">✓ Creative direction</div>
          <div className="rounded-2xl border border-hairline bg-white p-4">✓ Spend review</div>
        </div>
      </div>
